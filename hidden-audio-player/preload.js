...
function playAudio(src, volume, loop) {
    const sfx = new Howl({
        src,
        loop,
        volume
...
const playAudioThrottled = throttle(playAudio, 500);

window.addEventListener('DOMContentLoaded', () => {
    ipcRenderer.on("PLAYAUDIO", (event, file, volume, loop) => {
        playAudioThrottled(file, volume, loop);
    });
})
